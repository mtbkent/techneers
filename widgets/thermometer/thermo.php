...
$font = "../../fonts/Arimo-Bold/Arimo-Bold.ttf";

/*
else if (file_exists("/usr/local/fonts/ttf/arial.ttf")) {
	$font = "/usr/local/fonts/ttf/arial.ttf";
}
else if (file_exists("/Library/Fonts/Arial.ttf")) {
...
$t_message = isset($_GET['message']) ? $_GET['message'] : 0;

$finalimagewidth = 56 + max(strlen($t_max),strlen($t_current)) * 2;
$finalimage = imagecreateTrueColor(60+$finalimagewidth,205);
...
if ($t_current > $t_max) {
    ...
    $ybars = $t_max ? round(7 * ($t_current / $t_max)) : 0;
}
...
if ($font && (file_exists($font))) {
    imagettftext ($finalimage, $fsize, 0, 60, 155, $black, $font,$t_unit."0");                 // Write the Zero
    ...
    if ($t_current > $t_max) {
        ...
        if ($t_current == $t_max) {
			...
			$fypos = max($ypos + 15, 11 + (3 * $fsize));
            imagettftext ($finalimage, $fsize, 0, 60, $fypos, ($t_current > 0) ? $black : $red, $font, $t_unit."$t_current");  // Current < Max
        }
    }
	if ($t_message) {
		imagettftext ($finalimage, 8, 0, 60, 195, $red, $font, $t_message);
	}
}
